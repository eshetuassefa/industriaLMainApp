  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      if (response.success) {
        ...
        throw new Error(response.error?.message || "Failed to fetch urgent scans");
      }
    } catch (err) {
      setError(err.message || "Failed to fetch urgent scans");
      toast({
        title: "Error",
        description: err.message || "Failed to fetch urgent scans",
        variant: "destructive",
      });
    }
  }, [toast]);
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Urgent Scans</h1>
        <Button
      ...
      {requests.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <h3 className="text-lg font-medium text-gray-900">No Urgent Scans</h3>
            <p className="text-sm text-gray-500">
              There are currently no urgent scans to review.
            </p>
          </div>
        </div>
      ) : (
                ...
                {requests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <Badge
                        variant="secondary"
                        className={req.status === "PENDING" ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-800"}
                      >
                        {req.status}
                      </Badge>
                    </td>
      ...
      <Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}>
        <DialogContent className="sm:max-w-[800px]">
          <DialogHeader>
            ...
            <DialogDescription>
              View the complete details of this urgent radiology request
            </DialogDescription>
          </DialogHeader>
          {selectedReport && (
            <div className="py-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Badge
                    variant="secondary"
                    className={selectedReport.status === "PENDING" ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-800"}
                  >
