...
const UrgentScansList = () => {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      if (response.success) {
        const urgentRequests = response.data
          .filter((r) => r.urgency === "URGENT")
          .map(({ id, imagingType, bodyPart, status }) => ({
            id,
            imagingType,
            bodyPart,
            status,
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Doctor Name</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Radiologist Name</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Imaging Type</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Body Part</th>
              ...
              {requests.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50 cursor-pointer">
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${req.status === "PENDING" ? "bg-yellow-100 text-yellow-800" : req.status === "IN_PROGRESS" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"}`}>{req.status || "N/A"}</span>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm font-medium">
                    ...
                    >
                      {req.status === "COMPLETED" ? "View Results" : "Start Scan"}
                    </button>
                  </td>
                </tr>
              ))}
