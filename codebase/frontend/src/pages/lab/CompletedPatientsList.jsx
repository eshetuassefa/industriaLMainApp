    ...
    const fetchCompletedPatients = async () => {
      try {
        ...
        if (response.success) {
          ...
          const completedTests = response.data.filter(test => test.status === "COMPLETED");
          setPatients(completedTests);
        } else {
          throw new Error(response.error?.message || "Failed to fetch completed tests");
        }
      } catch (err) {
  ...
  if (patients.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <h2 className="text-xl font-semibold text-gray-700 mb-2">No Completed Tests</h2>
        <p className="text-gray-500 mb-4">There are currently no completed tests. Check back later or refresh the page.</p>
          ...
          {testDetails && (
            <div className="py-4 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-sm text-gray-500">Completed At</h3>
                  <p className="text-sm">{new Date(testDetails.results[0]?.completedAt).toLocaleDateString()}</p>
                </div>
                <div>
                ...
                <div>
                  <h3 className="font-semibold text-sm text-gray-500">Lab Technician</h3>
                  <p className="text-sm">
                    {testDetails.results[0]?.technician?.person?.firstName
                      ? `${testDetails.results[0].technician.person.firstName} ${testDetails.results[0].technician.person.middleName || ''} ${testDetails.results[0].technician.person.lastName || ''}`.trim()
                      : 'N/A'}
                  </p>
                </div>
              </div>
              ...
              {testDetails.results[0]?.values && (
                <div>
                  <h3 className="font-semibold text-sm text-gray-500 mb-2">Test Results</h3>
