...
import { getForwardedPatient, addMedicalRecord, getPrescriptionsByPatient, getLabTestRequests, getRadiologyRequests } from '../../services/provider.service';
...
const MedicalRecord = () => {
  ...
  const isChildRoute =
    matchPath('/provider/medical-record/:patientId/prescriptions', location.pathname) ||
    matchPath('/provider/medical-record/:patientId/lab-tests', location.pathname) ||
    matchPath('/provider/medical-record/:patientId/radiology-requests', location.pathname);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Fetch completed data from other services
      let completedPrescriptions = [];
      let completedLabResults = [];
      let completedRadiologyReports = [];

      try {
        // Fetch prescriptions for this patient
        const prescriptionsResponse = await getPrescriptionsByPatient(patientId);
        completedPrescriptions = prescriptionsResponse.data || [];
        console.log('Fetched prescriptions:', completedPrescriptions);
      } catch (error) {
        console.error('Error fetching prescriptions:', error);
      }

      try {
        // Fetch lab test requests for this patient
        const labRequestsResponse = await getLabTestRequests();
        const patientLabRequests = labRequestsResponse.data.filter(r => r.patientId === patientId);
        console.log('Fetched lab requests:', patientLabRequests);
        
        // Convert lab requests to lab results format
        completedLabResults = patientLabRequests.map(request => ({
          testName: request.testType?.name || request.testTypeId,
          testDate: new Date(request.requestedAt).toISOString(),
          resultValue: '', // Will be filled by lab
          unit: '',
          referenceRange: ''
        }));
      } catch (error) {
      ...
      try {
        // Fetch radiology requests for this patient
        const radiologyRequestsResponse = await getRadiologyRequests();
        const patientRadiologyRequests = radiologyRequestsResponse.data.filter(r => r.medicalRecord?.patientId === patientId);
        console.log('Fetched radiology requests:', patientRadiologyRequests);
        
        // Convert radiology requests to radiology reports format
        completedRadiologyReports = patientRadiologyRequests.map(request => ({
          imagingType: request.imagingType,
          reportText: request.notes || '',
          bodyPart: request.bodyPart || '',
          reportDate: new Date(request.createdAt).toISOString()
        }));
      } catch (error) {
      ...
      const recordData = {
        ...
        labResults: [
          // Include lab tests from form
          ...formData.labTests.map(test => ({
            testName: test,
          ...
          // Include completed lab results
          ...completedLabResults
        ],
        prescriptions: [
          // Include prescriptions from form
          ...
          // Include completed prescriptions
        ...
        radiologyReports: [
          // Include radiology requests from form
          ...formData.radiologyRequests.map(test => ({
            imagingType: test,
            reportText: formData.radiologyReason,
            bodyPart: '', // To be filled by radiology
            reportDate: new Date().toISOString()
          })),
          // Include completed radiology reports
          ...completedRadiologyReports
        ]
      };
    ...
    <>
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-8 mt-32">
